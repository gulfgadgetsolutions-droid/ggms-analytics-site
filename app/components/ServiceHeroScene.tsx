...
export default function ServiceHeroScene({ variant }: { variant: ServiceSceneVariant }) {
  return <div className="service-live-scene pointer-events-none absolute right-[4%] top-1/2 hidden h-[48%] w-[40%] -translate-y-1/2 overflow-hidden rounded-[2rem] border border-white/35 bg-slate-950/20 shadow-[0_32px_80px_rgba(15,23,42,0.32)] backdrop-blur-[3px] lg:block" aria-hidden="true"><Canvas camera={{ position: [0, 0, 5.4], fov: 42 }} dpr={[1, 1.4]} gl={{ alpha: true, antialias: true }}><ambientLight intensity={1.8} /><directionalLight position={[3, 4, 5]} intensity={2.1} color="#ffffff" /><pointLight position={[-2, 0, 3]} intensity={1.2} color="#22d3ee" />{variant === "strategy" && <StrategyScene />}{variant === "engineering" && <EngineeringScene />}{variant === "analytics" && <AnalyticsScene />}{variant === "science" && <ScienceScene />}{variant === "automation" && <AutomationScene />}{variant === "managed" && <ManagedScene />}</Canvas></div>;
}
